...
class CountyLine: SKScene, SKPhysicsContactDelegate {
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        messageLabel2.removeFromParent()
        ...
        for touch in touches {
            ...
            if nodeAtPoint(location).name == "Resume" {
                ...
                spawner = NSTimer.scheduledTimerWithTimeInterval(NSTimeInterval(randomBetweenNumbers(2, secNum: 4)), target: self, selector: #selector(self.spawnLowObstacles), userInfo: nil, repeats: true)
                spawner2 = NSTimer.scheduledTimerWithTimeInterval(NSTimeInterval(randomBetweenNumbers(3, secNum: 8)), target: self, selector: #selector(self.spawnHighObstacles), userInfo: nil, repeats: true)
    ...
    func getLabel() {
        ...
        messageLabel2 = self.childNodeWithName("tuts") as! SKLabelNode
    ...
    func playerDied() {
        ...
        dispatch_after(dispatchTime, dispatch_get_main_queue()) {
            ...
            for child in self.children {
                if child.name == "Pothole" || child.name == "knife" || child.name == "Barrel" || child.name == "Donuts" {
