...
class Ellis: SKScene, SKPhysicsContactDelegate {
    ...
    var animalArray = ["elephant","giraffe","hippo","panda","parrot","penguin","pig","rabbit","snake"]
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if nodeAtPoint(location).name == "ContinueTry" {
                ...
                spawner2 = NSTimer.scheduledTimerWithTimeInterval(NSTimeInterval(randomBetweenNumbers(3, secNum: 7)), target: self, selector: #selector(self.spawnHighObstacles), userInfo: nil, repeats: true)
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if firstBody.node?.name == "Player" && secondBody.node?.name == "animal" {
            secondBody.node?.removeFromParent()
            player.runAction(SKAction.playSoundFileNamed("Coin.wav", waitForCompletion: false))
            incrementScore()
        }
    ...
    func createLowObstacles() {
        ...
        let rand = Int(arc4random_uniform(UInt32(animalArray.count)))
        ...
        let obstacle3 = SKSpriteNode(imageNamed:animalArray[rand])
        obstacle3.name = "animal"
    ...
    func createHighObstacles() {
        ...
        let obstacle2 = SKSpriteNode(imageNamed:animalArray[rand])
        obstacle2.name = "animal"
        obstacle2.setScale(0.7)
    ...
    func getLabel() {
        ...
        messageLabel.text = "Collect 30 Zoo Animals"
    ...
    func levelComplete() {
        if levelsCompleted < 9 {
            NSUserDefaults.standardUserDefaults().setInteger(9, forKey: "LevelsCompleted")
            levelsCompleted = 9
        }
        if currentLevel < 9 {
    ...
    func playerDied() {
        ...
        dispatch_after(dispatchTime, dispatch_get_main_queue()) {
            ...
            for child in self.children {
                if child.name == "Pothole" || child.name == "knife" || child.name == "Barrel" || child.name == "animal" {
