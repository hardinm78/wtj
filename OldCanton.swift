...
class OldCanton:SKScene, SKPhysicsContactDelegate {
    ...
    func getLabel() {
        ...
        scoreLabel.text = "150M"
        messageLabel = self.childNodeWithName("Message") as! SKLabelNode
        messageLabel.text = "Survive for 150 meters"
    ...
    func levelComplete() {
        if levelsCompleted < 11 {
            NSUserDefaults.standardUserDefaults().setInteger(11, forKey: "LevelsCompleted")
            levelsCompleted = 11
        }
        if currentLevel < 11 {
