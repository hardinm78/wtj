...
class McDowell:SKScene, SKPhysicsContactDelegate {
    ...
    func getLabel() {
        ...
        scoreLabel.text = "125M"
        messageLabel = self.childNodeWithName("Message") as! SKLabelNode
        messageLabel.text = "Survive for 125 meters"
    ...
    func levelComplete() {
        if levelsCompleted < 8 {
            NSUserDefaults.standardUserDefaults().setInteger(8, forKey: "LevelsCompleted")
            levelsCompleted = 8
        }
        if currentLevel < 8 {
